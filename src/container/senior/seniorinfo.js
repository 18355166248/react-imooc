...
class SeniorInfo extends Component {
  ...
  render() {
      ...
      return ( <
          div > {
            redirect && redirect != pathname ? < Redirect to = {
              redirect
            }
            /> : null} <
            NavBar mode = "dark" > 牛人完善信息页 < /NavBar> <
            WingBlank > {
              this.props.user.msg === '' ? null : ( < div className = "error" > {
                  this.props.user.msg
                } < /div>) }</WingBlank >
                <
                ...
                /> <
                InputItem clear onChange = {
                  el => this.selectAvatar('desc', el)
                } > 求职岗位 < /InputItem> <
                TextareaItem onChange = {
                  el => this.selectAvatar('require', el)
                }
                title = "个人能力"
                autoHeight /
                >
